...
exports.shorten = function(json, res) {
    if (validator.isAlphanumeric(json.key) && validator.isURL(json.url)) {
                    ...
                    if (!err) {
                        if (rows.length > 0) {
                            ...
                            getKey(connection, group + user.short_key, json.url, function (err, key, exists) {
                                if (!err) {
                                    if (!exists) {
                                        connection.query('INSERT INTO urls SET ?', {id_user: user.id_user, key: key, url: json.url, created: 'NOW()', timeout: 'NOW()'});
                                    }

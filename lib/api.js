                ...
                connection.query('SELECT * FROM users WHERE `key` = "' + json.key + '"', function (err, rows) {
                    if (!err) {
                        if (rows.length > 0) {
                            ...
                            connection.query('SELECT `key`, `url`, `protection`, `protection_argument` as password, `created`, `timeout`, `clicks` FROM urls WHERE `id_user` = "' + user.id_user + '"', function (err2, rows2) {
                                if (!err2) {
                                    res.send({urls: rows2, status: 'ok'});
                                } else {
                                    res.send({status: 'error', error: exports.error(0)});
                ...
                connection.query('SELECT * FROM users WHERE `key` = "' + json.key + '"', function (err, rows) {
                    if (!err) {
                        if (rows.length > 0) {
                            ...
                            connection.query('SELECT `key`, `url`, `protection`, `protection_argument` as password, `created`, `timeout`, `clicks` FROM urls WHERE `id_user` = "' + user.id_user + '" AND `key` IN ("' + json.urls.split(',').join('","') + '")', function (err2, rows2) {
                                if (!err2) {
                                    res.send({urls: rows2, status: 'ok'});
                                } else {
                                    res.send({status: 'error', error: exports.error(0)});
